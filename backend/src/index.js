...
app.post('/api/auth/register', async (req, res) => {
  ...
  if (!email || !username || !password || password.length < 8) return res.status(400).json({ error: 'Email, Username und Passwort (mindestens 8 Zeichen) erforderlich' });
  try { const result = db.prepare('INSERT INTO users (email, username, password_hash) VALUES (?, ?, ?)').run(email.trim().toLowerCase(), username.trim(), await bcrypt.hash(password, 12)); res.status(201).json({ message: 'Registrierung erfolgreich. Admin muss verifizieren.', userId: result.lastInsertRowid }); }
...
app.post('/api/folders', auth, role('admin', 'moderator'), (req, res) => { if (!req.body.name?.trim()) return res.status(400).json({ error: 'Name erforderlich' }); const r = db.prepare('INSERT INTO folders (name, description, parent_id) VALUES (?, ?, ?)').run(req.body.name.trim(), req.body.description || '', req.body.parentId || null); res.status(201).json(db.prepare('SELECT * FROM folders WHERE id=?').get(r.lastInsertRowid)); });
...
app.post('/api/sources', auth, (req, res) => {
  ...
  const r = db.prepare(`INSERT INTO source_records (title,url,source_type,license,attribution,notes,created_by) VALUES (?,?,?,?,?,?,?)`).run(title.trim(), url.trim(), sourceType, license.trim(), attribution.trim(), notes.trim(), req.user.id);
